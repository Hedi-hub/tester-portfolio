# Base URLs
BASE_URL = "https://grocerymate.masterschool.com"

# Specific page URLs
HOME_PAGE_URL = BASE_URL
SHOP_PAGE_URL = BASE_URL + "/store"
PRODUCT_PAGE_URL = BASE_URL + "/product"
CART_PAGE_URL = BASE_URL + "/checkout"
CHECKOUT_PAGE_URL = BASE_URL + "/checkout"

# Authentication URLs
LOGIN_URL = BASE_URL + "/auth"
REGISTER_URL = BASE_URL + "/auth"

# Test Credentials
TEST_USERNAME = "John Doe Ex"
TEST_PASSWORD = "M@rketM@te25"
TEST_EMAIL = "johndoe@example1.com"

# Checkout Page Constants
DEFAULT_STREET_ADDRESS = "123 Main Street"
DEFAULT_CITY = "Berlin"
DEFAULT_POSTAL_CODE = "10115"

DEFAULT_CARD_NUMBER = "4111111111111111"
DEFAULT_NAME_ON_CARD = "John Doe"
DEFAULT_EXPIRATION = "12/25"
DEFAULT_CVV = "123"
